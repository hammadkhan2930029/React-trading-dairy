...
export const fetchUserCounts = createAsyncThunk(
    ...
    async (_, thunkAPI) => {
        try {
            const response = await api.get("/adminpanel/user_counts/");
...
export const fetchRegisteredUsers = createAsyncThunk(
    ...
    async (_, thunkAPI) => {
        try {
            const response = await api.get("/adminpanel/registered-users/");
            return response.data;
        } catch (err) {
...
export const fetchVerifiedUsers = createAsyncThunk(
    ...
    async (_, thunkAPI) => {
        try {
            const response = await api.get("/adminpanel/verified-users/");
            return response.data;
        } catch (err) {
...
const userListTypeslice = createSlice({
    ...
    reducers: {
        ...
        setUserImports: (state) => {
            state.formType = 996; // user import view
        },
        reset: (state) => {
            state.formType = null;
        },
...
export const { setBuyForm, setSellForm, setVerifiedusers, registeredUsers, reset, close_Trades, setUserImports } = userListTypeslice.actions;
