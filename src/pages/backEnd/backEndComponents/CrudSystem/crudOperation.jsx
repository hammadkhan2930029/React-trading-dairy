...
const CrudOperation = () => {
    ...
    const handleOpen = async (item) => {
        ...
        if (type === "BUY") {
            ...
            const holdingQty = item.previous_holding;
            formQty = holdingQty;
    ...
    useEffect(() => {
        setPage(0);
    }, [trades, searchText, selectTypeMain, selectType, fromDate, toDate]);

    // if user has holding then edit works
    useEffect(() => {
        const fetchDayTrades = async () => {
            try {
                const res = await api.get('/stocks/daytrade/');
                setDayTrades(res.data); 
            } catch (err) {
                console.error('Day trade fetch error', err);
            }
        };
    ...
    const isDayTrade = (trade) => {
        return dayTrades?.some(dt =>
            dt.stock_id === trade.stock?.id &&
            dt.cycle_id === trade.cycle_id
        );
    };

    const hasActiveHolding = (trade) => {
        const holdingExists = holdings?.some(h =>
            h.stock_id === trade.stock?.id &&
            h.cycle_id === trade.cycle_id &&
            h.holding_quantity > 0
        );

        const dayTradeExists = isDayTrade(trade);

        return holdingExists || dayTradeExists;
    };
    ...
    const filteredData = useMemo(() => {
        ...
        return trades.filter(item => {
            ...
            const tradeTypeMatch =
                selectType === '1' ||
                (selectType === '2' && item.trade_type === 'BUY') ||
                (selectType === '3' && item.trade_type === 'SELL');

            const tradeDate = item.trade_date ? new Date(item.trade_date) : null;
            const from = fromDate ? new Date(fromDate) : null;
            const to = toDate ? new Date(toDate) : null;

            const fromMatch = !from || (tradeDate && tradeDate >= from);
            const toMatch = !to || (tradeDate && tradeDate <= to);

            return symbolMatch && dayTradeMatch && tradeTypeMatch && fromMatch && toMatch;
        });
            ...
            <div className={styles.crud_main}>
                <div className={styles.top_input_fields}>
                    <div className={styles.top_btn}>
                        <button className={styles.top_btn_buy} onClick={() => navigate('/buy')}>
                            Buy
                        </button>
                        <button className={styles.top_btn_buy} onClick={() => navigate('/sell')}>
                            Sell
                        </button>
                    </div>
                </div>
