...
export const SellForm = () => {
    ...
    const [minBuyDate, setMinBuyDate] = useState("");
    ...
    const fetchStocks = async (dayTrade) => {
        try {
        const url =
            dayTrade === 1 ? "/stocks/daytrade/" : "/holdings/userholdings/";
    ...
    return (
        ...
        >
            ...
            >
                ...
                const fetchTradeDate = async (stockId, dayTrade) => {
                    if (!stockId) return;

                    try {
                        const response = await api.get(`/stocks/buydate/?stock_id=${stockId}&day_trade=${dayTrade}`);
                        const data = response.data;

                        if (data.trade_date) {
                            setMinBuyDate(data.trade_date);
                            setFieldValue("trade_date", data.trade_date);
                            setFieldValue("sett_date", data.settlement_date);
                        }
                    } catch (error) {
                        
                    }
                };  
                //  When user toggles Day Trade (Yes/No), reset all dependent fields
                useEffect(() => {
                    // Clear trade-specific fields when toggling
                    setFieldValue("stock_id", "");
                    setFieldValue("trade_date", "");
                    setFieldValue("sett_date", "");
                    setFieldValue("holding_quantity", 0);
                    setFieldValue("avg_buying", 0);
                    setFieldValue("sell_QTY", 0);
                    setFieldValue("sell_rate", 0);
                    setFieldValue("sell_amount", 0);
                    setFieldValue("sell_broker_amount", 0);
                    setFieldValue("sell_cdc_amount", 0);
                    setFieldValue("sell_sst_amount", 0);
                    setFieldValue("net_amount", 0);

                    // Fetch new stock list
                    fetchStocks(values.day_trade);
                }, [values.day_trade]);
                ...
                //  When stock changes, update quantities/avg buying
                useEffect(() => {
                    if (!values.stock_id) return;
                    // fetchTradeDate(values.stock_id);
                    fetchTradeDate(values.stock_id, values.day_trade);
                    
                    const selected = stocks.find(
                        (s) => s.stock_id === values.stock_id || s.id === values.stock_id
                    );

                    if (selected) {
                        if (values.day_trade === 0) {
                            setFieldValue(
                                "trade_date",
                                selected.trade_date|| 0
                            );
                            setFieldValue(
                                "holding_quantity",
                                selected.holding_quantity || 0
                            );
                            setFieldValue(
                                "avg_buying",
                                selected.weighted_avg_buying || 0
                            );
                        } else {
                            setFieldValue(
                                "holding_quantity",
                                selected.holding_quantity_quantity || selected.quantity || 0
                            );
                            setFieldValue("avg_buying", selected.avg_buying || 0);
                        }
                    }
                }, [values.stock_id, values.day_trade, stocks, setFieldValue]);                                  
                    ...
                    <form onSubmit={handleSubmit} className='form_div_sell'>
                        <div className='form-main-sell'>
                            ...
                            <div className='btn_div'>
                                ...
                                <div className='switchBtn_div_sell'>
                                    <button className='switchBtn_sell' onClick={() => navigate('/buy')}>Buy </button>
                                </div>
                            </div>
                            <div >
                                <Box 
                                    ...
                                    <div className='form_sell'>
                                        ...
                                        <Autocomplete
                                            ...
                                            getOptionLabel={(option) =>
                                                option.stock_symbol || option.symbol || ""
                                            }
                                            value={
                                                stocks.find(
                                                    (item) =>
                                                        (item.stock_id || item.id) === values.stock_id
                                                ) || null
                                            }
                                            onChange={(event, newValue) => {
                                                setFieldValue(
                                                    "stock_id",
                                                    newValue ? (newValue.stock_id || newValue.id) : ""
                                                );
                                            }}
                                            isOptionEqualToValue={(option, value) =>
                                                (option.stock_id || option.id) === (value.stock_id || value.id)
                                            }
                                            renderInput={(params) => (
                                                <TextField
                                                    {...params}
